...
FROM
  hc848.MITRE_PATIENT p
  INNER JOIN hc848.MITRE_PATIENT_IMMUNIZATION i ON p.PATIENT_ID = i.PATIENT_ID
WHERE
...
FROM hc848.NITRE_PATIENT p
JOIN hc848.NITRE_PATIENT_IMMUNIZATION pv ON p.PATIENT_ID = pv.PATIENT_ID
JOIN hc848.NITRE_IMMUNIZATION v ON pv.IMMUNIZATION_CODE = v.IMMUNIZATION_CODE
JOIN hc848.NITRE_CITY nc ON p.LIVING_PLACE_COUNTRY_ID = nc.COUNTRY_ID
JOIN hc848.NITRE_COUNTY co ON nc.COUNTY_ID = co.COUNTY_ID
JOIN hc848.NITRE_RACE r ON p.RACE_ID = r.RACE_ID
...
FROM (
    ...
    FROM hc848.MITRE_PATIENT P
    JOIN hc848.MITRE_PATIENT_MEDICATION MED ON MED.PATIENT_ID = P.PATIENT_ID
    JOIN hc848.MITRE_PATIENT_IMMUNIZATION IMM ON IMM.PATIENT_ID = P.PATIENT_ID
...
FROM (
    SELECT 
        ...
        SUM(MED.TOTAL_COST) AS TOTAL_COST
    FROM hc848.NITRE_PATIENT P
    JOIN hc848.NITRE_PATIENT_MEDICATION MED ON MED.PATIENT_ID = P.PATIENT_ID
    JOIN hc848.NITRE_MEDICATION MD ON MED.MEDICATION_CODE = MD.MEDICATION_CODE
    JOIN hc848.NITRE_RACE NR ON P.RACE_ID = NR.RACE_ID
    JOIN hc848.NITRE_PATIENT_IMMUNIZATION IMM ON IMM.PATIENT_ID = P.PATIENT_ID
    JOIN hc848.NITRE_CITY nc ON p.LIVING_PLACE_COUNTRY_ID = nc.COUNTRY_ID
    JOIN hc848.NITRE_COUNTY co ON nc.COUNTY_ID = co.COUNTY_ID
...
FROM
    hc848.MITRE_PATIENT p
    JOIN hc848.MITRE_PATIENT_PROCEDURE pr ON p.PATIENT_ID = pr.PATIENT_ID
...
WHERE
    ...
    AND p.PATIENT_ID IN (
        SELECT i.PATIENT_ID
        FROM hc848.MITRE_PATIENT_IMMUNIZATION i
...
-- SQL (NITRE):	
    ...
    SUM(pr.BASE_COST) AS TOTAL_BASE_COST
FROM
    hc848.NITRE_PATIENT p
    JOIN hc848.NITRE_PATIENT_PROCEDURE pr ON p.PATIENT_ID = pr.PATIENT_ID
    JOIN hc848.NITRE_REASON nre ON pr.REASON_CODE = nre.REASON_CODE
    JOIN hc848.NITRE_RACE NR ON P.RACE_ID = NR.RACE_ID
    JOIN hc848.NITRE_CITY nc ON p.LIVING_PLACE_COUNTRY_ID = nc.COUNTRY_ID
    JOIN hc848.NITRE_COUNTY co ON nc.COUNTY_ID = co.COUNTY_ID
WHERE co.COUNTY_ID = 9
    ...
    AND p.PATIENT_ID IN (
        SELECT i.PATIENT_ID
        FROM hc848.NITRE_PATIENT_IMMUNIZATION i
        WHERE i.IMMUNIZATION_CODE IN ('52', '43', '83', '8')
        )
