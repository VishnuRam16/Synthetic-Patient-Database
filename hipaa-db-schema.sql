...
BEGIN
    IF INSERTING THEN
        ...
        VALUES ('NITRE_PATIENT', 'INSERT', SYS_CONTEXT('USERENV', 'SESSION_USER'), TO_CLOB(:NEW.PATIENT_ID || ',' || :NEW.FIRST_NAME || ',' || :NEW.LAST_NAME));
    ELSIF UPDATING THEN
        ...
        VALUES ('NITRE_PATIENT', 'UPDATE', SYS_CONTEXT('USERENV', 'SESSION_USER'), 
                TO_CLOB(:OLD.PATIENT_ID || ',' || :OLD.FIRST_NAME || ',' || :OLD.LAST_NAME),
                TO_CLOB(:NEW.PATIENT_ID || ',' || :NEW.FIRST_NAME || ',' || :NEW.LAST_NAME));
    ELSIF DELETING THEN
        INSERT INTO NITRE_AUDIT_LOG (TABLE_NAME, OPERATION, USER_ID, OLD_VALUE)
        VALUES ('NITRE_PATIENT', 'DELETE', SYS_CONTEXT('USERENV', 'SESSION_USER'), TO_CLOB(:OLD.PATIENT_ID || ',' || :OLD.FIRST_NAME || ',' || :OLD.LAST_NAME));
    END IF;
END;
/
